...
const CategoryListing = () => {
  ...
  const path = pathSegments[pathSegments.length - 1] === 'categories' ? 'category' : 'brand';
  console.log(pathSegments, path);

  const [sortBy, setSortBy] = useState<string>('Title: A → Z');
  const [query, setQuery] = useState<string>(location.search.replace('?q=', '') || '');
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <h2 className='text-2xl font-semibold capitalize'>{path} Listing</h2>
        <NavLink
      ...
      {!loading && data?.totalDocs ? <CategoryList items={data?.docs} /> : null}
    </div>
  );
};
