...
export default function UserCard({ items }: { items: UserType[] }) {
  ...
  const handleRoleChange = async (id: string, role: UserRoleEnum) => {
    try {
          ...
          {users.map((category, index) => (
            <tr
              ...
              <td className='py-3 px-4 text-nowrap'>
                {format(new Date(category.createdAt || new Date()), 'MMM d, yyyy')}
              </td>
              <td className='px-4'>
                <select
                  ...
                  id='role'
                  value={category.role}
                  onChange={(e) => handleRoleChange(category._id, e.target.value as UserRoleEnum)}>
              ...
              <td className='px-4'>
                <select
                  ...
                  onChange={(e) => handleStatusChange(category._id, e.target.value as StatusEnum)}>
