...
const CategoryForm = ({ item }: { item: CategoryType | null }) => {
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      const endpoint = `/category${item?._id ? `/${item._id}` : ''}`;
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className='space-y-6 text-gray-700 max-w-3xl mx-auto'>
        <h2 className='text-xl font-bold text-gray-700 capitalize'>Category Form</h2>
        <div className='flex gap-5 max-sm:flex-col sm:items-end'>
          ...
          <div className='grid gap-2'>
            <label htmlFor='status' className='capitalize text-sm font-medium text-gray-700'>
              Status
            </label>
            <Select
              className='right-0'
