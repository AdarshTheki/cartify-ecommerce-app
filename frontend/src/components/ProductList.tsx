      ...
      <table className='w-full'>
        <thead>
          <tr className='border-b border-gray-300 text-slate-700'>
            ...
            <th className='text-left py-3 px-4'>Category ({categories.length})</th>
            ...
            <tr
              ...
              <td className='py-3 px-2 max-w-fit'>
                <select
                  ...
                  id='brand'
                  value={product.brand}
                  onChange={(e) => handleBrandChange(product._id, e.target.value)}>
                  {brands.map((i, index) => (
                    <option className='capitalize' key={i} value={i}>
                      {index + 1}. {i}
                    </option>
              ...
              <td className='px-2'>
                <select
                  ...
                  onChange={(e) =>
                    handleStatusChange(product._id, e.target.value as ProductStatusEnum)
                  }>
                  {statusOptions.map((i) => (
                    <option className='capitalize' key={i} value={i}>
                      {i}
